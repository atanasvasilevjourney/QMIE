...
- **Desk `Sync: Failed to fetch` / `desk API unreachable`:** the browser never reached FastAPI. Open `http://127.0.0.1:5173` (Vite `/qmie` → `:8080`). The UI now also retries `http://127.0.0.1:8080` (CORS). Empty radar / Binance 451 is a **different** problem — see `docs/streaming-data-sources.md`. Do **not** add Hyperliquid trading APIs. A Vercel `404: NOT_FOUND` with status Ready is an empty static deploy of the monorepo root — not a scanner host. See `docs/deployment.md` (Vercel = desk UI only).
- **Secrets:** Discord/Telegram/Slack and `WEBHOOK_SECRET` are optional for desk UI + journal. HMAC webhook posts need a matching secret from `python/.env`. Slack tokens stay on the scanner host (Render/Docker), not Vercel.
- **Desk tabs:** Landing is **ORBIT** (Orbis Universe 3D). **OPS** holds Trend Radar plus separate tables for TEMA scanner vs daily breakout (long **and** short) vs **EXIT** (paper close + PnL). **SCREENS** is the combo review list (`GET /screens`: unique symbols from 4h A/A+, daily breakout, radar coils, ranked book). Space next, Shift+Space flags a local focus list (not a fill). **CHARTS** is SVG equity + closed-bar price with entry/exit/SL/TP marks (no JS chart library; `/charts/book` and `/charts/price` load only while CHARTS or SCREENS is open). **GUIDE** is the trading-guide module. Do not put the 3D universe back on OPS. Row **DETAILS** expands stored alert/radar fields; it is not a historical HL replay (a 67–68k BTC higher-low only appears if that closed bar actually graded A/A+). Equities Deep View screening workflows map to existing QMIE views in `docs/deepview-screening-map.md` — do not port CAN SLIM, IPO screens, or a custom `W_*` filter DSL. KovaView skip gates (`too_late`, BTC-RED `buys_allowed`, cooldown) are **not** on the native checklist — they cut winners on the 4h A/A+ slice (`docs/kovaview-overlay-backtest.md`). Radar BTC color on the briefing card is display only. Do not Optuna per ticker or import SMA20 trails / ATR×2.5 as the paper exit (`docs/kovaview-equity-map.md`).
...
- **Frozen OOS:** `docs/backtest-baseline.md` is the TMA 9/90/199 walk-forward. Re-run with `python -m backtest.run --start 2024-01-01 --split 2025-01-01 --min-adx 20 --min-atr-pct 0.4 --max-atr-pct 4.0` (Vision cache, not `fapi`). Do not retune `W_*` on that sample. One-knob proposal: `strategy/reviews/2026-08-17.md` (`SCAN_TIMEFRAMES=4h`, not applied). `strategy/reviews/2026-08-25.md` restates that hold — the review CLI's next catalog item is `sig_min_adx` 0→20 because `scan_timeframes` was already proposed; do not apply ADX until 4h is live and measured. Do **not** add `1d` to `SCAN_TIMEFRAMES` — daily A/A+ OOS loses (ungated E[R] −0.273; see `docs/kovaview-overlay-backtest.md`). Journal `success` on this desk is mostly paper/qa fills, not a 30-fill manual book.
