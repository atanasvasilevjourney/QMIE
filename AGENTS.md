...
- **Desk tabs:** Landing is **ORBIT** (Orbis Universe 3D). **OPS** holds Trend Radar plus separate tables for TEMA scanner vs daily breakout vs **EXIT** (paper close + PnL). **CHARTS** is SVG equity + closed-bar price with entry/exit/SL/TP marks (no JS chart library; `/charts/book` and `/charts/price` load only while that tab is open). **GUIDE** is the trading-guide module. Do not put the 3D universe back on OPS. Row **DETAILS** expands stored alert/radar fields; it is not a historical HL replay (a 67–68k BTC higher-low only appears if that closed bar actually graded A/A+).
