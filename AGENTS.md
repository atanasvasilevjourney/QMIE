...
- **Desk `Sync: Failed to fetch`:** the browser never reached the FastAPI proxy. Open `http://127.0.0.1:5173` (Vite proxies `/qmie` → `:8080`). Direct `:8080` is JSON only. Empty radar / `data_source=binance` with 451 is a **different** problem (exchange geo-block), not this banner.
